...
export const getAllVideos = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const videos = await VideoModel.find(filter)
      .select(
        'title videoThumbnail videoUrl isPublic publishedDate totalView videoType createdAt slug'
      )
      .limit(limit)
      .skip(skip)
...
export const addVideo = async (req: CustomRequest, res: Response) => {
  ...
  try {
    ...
    if (playlist && playlist.trim() !== '' && mongoose.Types.ObjectId.isValid(playlist)) {
...
export const deleteVideo = async (req: CustomRequest, res: Response): Promise<Response> => {
...
export const getUserVideoCount = async (req: Request, res: Response): Promise<Response> => {
  try {
